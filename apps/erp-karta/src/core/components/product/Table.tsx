...
const TableProduct: React.FunctionComponent<Props> = ({
  typeId,
  data,
  loading,
  count,
  ...
  const [selected, setSelected] = React.useState<z.infer<typeof Product> | null>(null)
  ...
  const onEdit = React.useCallback(async (id: number) => {
    await router.push(`/admin/products/update/[typeId]/[id]`, `/admin/products/update/${typeId}/${id}`)
  }, [typeId])
